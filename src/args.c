...
void usage (void)
{
  ...
  fprintf (stdout, "  -i, --stitch                          concatenate reads by reversing/complementing reverse read\n");
...
int decode_switches (int argc, char * argv[], struct user_args * sw)
{
  ...
  while ((opt = getopt_long(argc, argv, "b:ef:g:hj:m:n:o:p:q:r:s:t:u:v:y:c:zki", long_options, &oi)) != -1)
   {
     switch (opt)
      {
        case 'b':
